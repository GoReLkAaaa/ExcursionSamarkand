...
class MainDeskription(models.Model):
    ...
    image_city = models.ImageField(upload_to='images/', blank=True, null=True,
...
class CatalogEkskursii(models.Model):
    name_excursion = models.CharField(max_length=100,
                verbose_name='Название экскурсии')
    price_excursion = models.IntegerField(
                verbose_name='Цена экскурсии')
    image_excursion = models.ImageField(upload_to='image_excursion/',
                verbose_name='Картинка экскурсии')
    main = models.ForeignKey(MainDeskription, on_delete=models.CASCADE, related_name='main',
                verbose_name='К какому городу относится')
...
class ModalText(models.Model):
    ...
    how_many_people = models.IntegerField(
                verbose_name='Стоимость за сколько человек')
    price_russian = models.IntegerField(
                verbose_name='Цена за русскую поездку')
    price_english = models.IntegerField(
                verbose_name='Цена за английскую поездку')
    what_is_not_included_in_price = models.CharField(max_length=200,
                verbose_name='Что не входит в стоимость')
    modal_ekskurs_name = models.ForeignKey(CatalogEkskursii, on_delete=models.CASCADE, related_name='modal_texts',
